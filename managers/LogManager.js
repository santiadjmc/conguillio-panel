...
class LogManager {
    ...
    error(source, message) {
        ...
        return console.log(`[${source.toUpperCase()}][ERROR]: ${message}`.brightRed);
    }
    /**
     * @param {string} source
     * @param {string} message
    ...
    info(source, message) {
        ...
        if (typeof message !== "string") throw new TypeError(`Source parameter must be of type string, received ${typeof message}`);
        if (!this.sources.some(s => s.toLowerCase() === source.toLowerCase())) throw new RangeError("Unknown source");
        return console.log(`[${source.toUpperCase()}][INFO]: ${message}`.grey);
    }
    /**
     * @param {string} source
     * @param {string} message
    ...
    success(source, message) {
        ...
        if (typeof message !== "string") throw new TypeError(`Source parameter must be of type string, received ${typeof message}`);
        if (!this.sources.some(s => s.toLowerCase() === source.toLowerCase())) throw new RangeError("Unknown source");
        return console.log(`[${source.toUpperCase()}][SUCCESS]: ${message}`.brightGreen);
    }
    /**
     * @param {string} source
     * @param {string} message
    ...
    warn(source, message) {
        ...
        if (typeof message !== "string") throw new TypeError(`Source parameter must be of type string, received ${typeof message}`);
        if (!this.sources.some(s => s.toLowerCase() === source.toLowerCase())) throw new RangeError("Unknown source");
        return console.log(`[${source.toUpperCase()}][WARNING]: ${message}`.brightYellow);
    }
}
